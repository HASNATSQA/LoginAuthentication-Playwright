...
function isDNVerificationMessage(msg: any): boolean {
  ...
  // Check if message matches DN verification code format: "(code) is your DN verification code. Just FYI, it'll expire in 2 minutes."
...
async function waitForNewSms(
  ...
  for (let i = 0; i < maxAttempts; i++) {
    ...
    for (const msg of filteredMessages) {
      ...
      // Only accept messages received under 30 seconds
      if (ageMs < MIN_AGE_MS) {
...
/* -------------------- UI Helpers -------------------- */
async function tryClickAny(page: Page, selectors: string[]): Promise<boolean> {
...
async function logoutFromProfile(page: Page): Promise<void> {
  ...
  try {
    const profileButton = page.getByRole('button', { name: 'Welcome Hasnat Ahmad, M.D.' });
    ...
    try {
      const welcomeButton = page.getByRole('button', { name: /Welcome/i });
      await welcomeButton.waitFor({ state: 'visible', timeout: 5000 });
      await welcomeButton.click();
      menuOpened = true;
      console.log('Opened profile menu (partial match)');
    } catch {
      ...
      for (let i = 0; i < count; i++) {
        ...
        if (text && (text.includes('Welcome') || text.includes('Hasnat'))) {
          ...
          await button.click();
          menuOpened = true;
          console.log(`Opened profile menu using button: "${text.trim()}"`);
          break;
        }
      }
    }
  ...
  if (!menuOpened) {
    menuOpened = await tryClickAny(page, [
      '.mat-mdc-menu-trigger.profile_pic', 'button.mat-mdc-menu-trigger.profile_pic',
      '.mat-mdc-menu-trigger', 'button.mat-mdc-menu-trigger', '.profile_pic', 'button:has-text("Welcome")'
    ]);
  }
  
  if (!menuOpened) throw new Error('Could not find or click profile menu button');
  
  await page.waitForTimeout(500);
  
  let logoutClicked = false;
  try {
    const logoutItem = page.getByRole('menuitem', { name: 'exit_to_app Logout' });
    await logoutItem.waitFor({ state: 'visible', timeout: 3000 });
    await logoutItem.click();
    logoutClicked = true;
    console.log('Clicked logout');
  } catch {
    logoutClicked = await tryClickAny(page, [
      '[role="menuitem"]:has-text("Logout")', 'mat-menu-item:has-text("Logout")',
      '.alignment-profile', 'text=Logout', 'button:has-text("Logout")',
      'a:has-text("Logout")', '*:has-text("exit_to_app Logout")'
    ]);
  }
  
  if (!logoutClicked) throw new Error('Could not find or click logout menu item');
...
test('DoctorNow login: SMS 2FA (robust)', async ({ context }) => {
  ...
  if (!DOC_EMAIL || !DOC_PASSWORD) {
    const missing = [(!DOC_EMAIL && 'DOC_EMAIL'), (!DOC_PASSWORD && 'DOC_PASSWORD')].filter(Boolean);
    throw new Error(`Missing required environment variables: ${missing.join(', ')}`);
  }

  console.log(`📱 Using SMS URL: ${RECEIVE_SMS_URL}`);
  console.log(`📱 Looking for DN verification code messages (format: "(code) is your DN verification code...")`);
  ...
  try {
    ...
    await logoutFromProfile(appPage);
